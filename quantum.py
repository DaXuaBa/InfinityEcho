...
connection = pymysql.connect(
    ...
    port=22400,
    user="root",
    password="kZQzidMFRrYOenGUtMpplvqxIKFZaQfR",
    database="apify"
)
...
def get_data_from_url():
    try:
        ...
        code = "apify_api_RiD1m2rnJRRJBb8evfT5HkRIRyjVMB3RV3Tw"
        url = f"https://api.apify.com/v2/acts/{actors}/runs?token={code}"
...
def insert_data_to_database():
    try:
        ...
        for x in item['data']['items']:
            ...
            # Câu lệnh SQL để kiểm tra xem defaultDatasetId đã tồn tại trong bảng A chưa
            check_sql = "SELECT COUNT(*) FROM A WHERE cloud = %s"
            ...
            if result[0] == 0:  # Nếu defaultDatasetId không tồn tại trong bảng
                # Câu lệnh SQL để chèn dữ liệu vào bảng A
                insert_sql = "INSERT INTO A (cloud) VALUES (%s)"
...
async def call_api():
    ...
    try:  
        ...
        sql_select = "SELECT id, cloud FROM A WHERE status = 0"
        cursor.execute(sql_select)
        records = cursor.fetchall()

        # Lặp qua từng bản ghi
        for record in records:
            record_id, cloud = record

            # Gọi API với default_dataset_id là cloud
            url = f"https://api.apify.com/v2/datasets/{cloud}/items"
            response = requests.get(url)

            # Kiểm tra xem request có thành công không
            if response.status_code == 200:
                # Đổi status của bản ghi từ 0 thành 1
                sql_update = "UPDATE A SET status = 1 WHERE id = %s"
                cursor.execute(sql_update, (record_id,))
                
                for x in response.json():
                    tweet_id = x['id']
                    # Kiểm tra xem tweet_id đã tồn tại trong MySQL hay chưa
                    sql_check = "SELECT COUNT(*) FROM B WHERE tweet_id = %s"
