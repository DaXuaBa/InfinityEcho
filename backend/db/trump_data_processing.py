...
def insert_data_to_database(db: Session):
    try:
        ...
        for x in item['data']['items']:
            ...
            exists = db.query(TrumpDatasetStatus).filter(TrumpDatasetStatus.dataset_id == defaultDatasetId).first()
            if not exists:
                new_status = TrumpDatasetStatus(dataset_id=defaultDatasetId)
                db.add(new_status)
                db.commit()
...
def call_api_trump(db: Session):
    print("Task in progress")
    insert_data_to_database(db)
    try:
        # Lấy các bản ghi từ cơ sở dữ liệu có status = 0
        records = db.query(TrumpDatasetStatus).filter(TrumpDatasetStatus.status == 0).all()
        ...
        for record in records:
            ...
            if response.status_code == 200:
                ...
                for x in response.json():
                    ...
                    exists = db.query(TrumpTweetData).filter(TrumpTweetData.tweet_id == tweet_id).first()
                    if not exists:
